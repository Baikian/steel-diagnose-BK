...
import { brushPre1,brushPre, addElement, updateElement, updateAsyncElement, updateStyles, stringify } from "./element";
...
export default class Parallel extends SuperGroupView {
    constructor({
        ...
        this.newkeys=[...this.keys, 'cooling']
    ...
    joinData(value,mid) {
        ...
        this.barmid = d3.map(this._keys, d => d3.map(this.mid, index => index[d]))
        // console.log('this.barmid',this.barmid)
        ...
        this.binmid = d3.map(this.keys, (d, i) => { return d3.bin().thresholds(10)(this.barmid[i])});
        // console.log('this.binmid ',this.binmid);
        ...
        let barScale1 = d3.map(this.binmid, array => d3.scalePow().domain([0, 1, d3.max(d3.map(array, d => d.length))]).range([0, 5, 35]))
        let parpallelDomain1 = d3.map(this.binmid, d => [d[0].x0, d.slice(-1)[0].x1]);
        let parpallelDomain = d3.map(this.barbin, d => [d[0].x0, d.slice(-1)[0].x1]);
        // console.log('parpallelDomain',parpallelDomain);
        this._x = new Map(Array.from(this._keys, (key, index) => [key, d3.scaleLinear(parpallelDomain[index], this.xRange).clamp(true)]));
        let xscle= new Map(Array.from(this._keys, (key, index) => [key, d3.scaleLinear(parpallelDomain1[index], this.xRange).clamp(true)]));
        xscle.set('status_cooling', d3.scaleLinear([-1, 2], this.xRange))
        this._x.set('status_cooling', d3.scaleLinear([-1, 2], this.xRange))
        // console.log('this._x',this._x);
        // console.log('this.xscle',xscle);
        this._y = d3.scalePoint(this._newkeys, [this._margin.top, this.height - this._margin.bottom])
        ...
        this.area = d3.area()
            .y(([key]) => this._y(key))
            .x0(([key, value]) => xscle.get(key)(value[0]))
            .x1(([key, value]) => xscle.get(key)(value[1])),
    ...
    render() {
        ...
        // this._container.attr('transform',`translate(50,0)`)
        ...
        const lineG = this._container.append('g')

        lineG.selectAll('.parallelPath')
            .data(Object.values(this.brushdata).sort((a, b) => d3.ascending(a['upid'], b['upid'])))
            .join(
                enter => this.#enterLine(enter),
                update => update,
                exit => exit
            )
        this._initBrush()
    ...
    _initBrush() {
        ...
        this._brushG
            .append('rect')
            .attr('transform', `translate(${0},${this.height - this._margin.bottom - 15})`)
            .attr('x', 20)
            .attr('y', 10)
    ...
    _brushSlider() {
        ...
        let rectBarExample = group.append('g').attr('class', 'rectBarG' + item)
            .attr('transform', `translate(45,${this._y(this._keys[item])})`)
            .attr('custom-handle', function (d) {
    ...
    #enterLine(group) {
        const that = this;
        let LineExample = group.append('g').attr('class', 'parallelPath')
            .attr('transform', `translate(45,10)`)
            .attr('custom-handle', function (d) {
                let LineInstance = new line({ width: this._width , height: 70 }, d3.select(this), that.line, that._newkeys,that._brushSelection,group,that._rootparent)
                LineInstance.render()
            })
    }
    // #enterArea(group) {
    ...
    //         .attr('transform', `translate(45,10)`)
    //         .attr('custom-handle', function (d) {
    //             let LineInstance = new area({ width: this._width , height: 70 }, d3.select(this), that.line, that._newkeys,group)
    //             LineInstance.render()
    //         })
    // }
}
